...
{
    internal class dependyy
    {
        private List<Service_Discript> serviceDescrList = new List<Service_Discript>();
        
        public void AddSingleton<TService, TImplementation>() where TImplementation : TService
        {
            serviceDescrList.Add(new Service_Discript(typeof(TService), typeof(TImplementation), ServiceLifeTime.Singletone));
        }

        public void AddTransient<TService, TImplementation>() where TImplementation : TService
        {
            serviceDescrList.Add(new Service_Discript(typeof(TService), typeof(TImplementation), ServiceLifeTime.Transient));
        }
